...
namespace Tinke.Tools.Cryptography
{
    class SAEncryptor
    {
        ...
        const uint MAGIC30 = 0x72636E65;
        const uint MAGIC34 = 0x6A624F79;
        ...
        void Encrypt(uint[] magic, ref uint arg1, ref uint arg2)
        {
            uint a, b, c;
            a = arg1;
            b = arg2;
        ...
        void Decrypt(uint[] magic, ref uint arg1, ref uint arg2)
        {
            uint a, b, c;
            a = arg1;
            b = arg2;
        ...
        void Encrypt(uint[] magic, ref UInt64 cmd)
        {
            uint arg1 = (uint)(cmd >> 32);
            uint arg2 = (uint)(cmd >> 0);
            this.Encrypt(magic, ref arg1, ref arg2);
            cmd = ((UInt64)arg1 << 32) + arg2;
        }

        void Decrypt(uint[] magic, ref UInt64 cmd)
        {
            uint arg1 = (uint)(cmd >> 32);
            uint arg2 = (uint)(cmd >> 0);
            this.Decrypt(magic, ref arg1, ref arg2);
            cmd = ((UInt64)arg1 << 32) + arg2;
        }

        void UpdateHashtable(uint[] magic, byte[] arg1/*[8]*/)
        {
            for (int j = 0; j < 18; j++)
            {
                uint r3 = 0;
            ...
            {
                this.Encrypt(magic, ref tmp1, ref tmp2);
                magic[i + 0] = tmp1;
                magic[i + 1] = tmp2;
            }
        ...
        {
            this.Encrypt(magic, ref a[2], ref a[1]);
            this.Encrypt(magic, ref a[1], ref a[0]);
            byte[] ab = new byte[8];
            Array.Copy(BitConverter.GetBytes(a[0]), 0, ab, 0, 4);
            Array.Copy(BitConverter.GetBytes(a[1]), 0, ab, 4, 4);
            this.UpdateHashtable(magic, ab);
        }

        void Init1(uint cardheader_gamecode)
        {
            for (int i = 0; i < 1024 + 18; i++) this.card_hash[i] = BitConverter.ToUInt32(encr_data, 4 * i);
            arg2[0] = cardheader_gamecode;
            arg2[1] = cardheader_gamecode >> 1;
            arg2[2] = cardheader_gamecode << 1;
            this.Init2(card_hash, arg2);
            this.Init2(card_hash, arg2);
        }

        void Init0(uint cardheader_gamecode)
        {
            this.Init1(cardheader_gamecode);
            this.Encrypt(card_hash, ref global3_x04, ref global3_x00);
            global3_rand1 = global3_x00 ^ global3_x04;      // more RTC
            global3_rand3 = global3_x04 ^ 0x0380FEB2;
            this.Encrypt(card_hash, ref global3_rand3, ref global3_rand1);
        }

        #endregion

        #region Main crypt methods

        public static void DecryptSecureArea(uint cardheader_gamecode, byte[] data)
        {
            SAEncryptor dec = new SAEncryptor();
            dec.DecryptArm9(cardheader_gamecode, data);
        }

        public static void EncryptSecureArea(uint cardheader_gamecode, byte[] data)
        {
            SAEncryptor enc = new SAEncryptor();
            enc.EncryptArm9(cardheader_gamecode, data);
        }

        public void DecryptArm9(uint cardheader_gamecode, byte[] data)
        {
            uint[] p = new uint[data.Length / 4];
            for (int i = 0; i < p.Length; i++) p[i] = BitConverter.ToUInt32(data, 4 * i);

            this.Init1(cardheader_gamecode);
            this.Decrypt(card_hash, ref p[1], ref p[0]);
            arg2[1] <<= 1;
            arg2[2] >>= 1;
            this.Init2(card_hash, arg2);
            this.Decrypt(card_hash, ref p[1], ref p[0]);
            ...
            while (size > 0)
            {
                this.Decrypt(card_hash, ref p[pOffset + 1], ref p[pOffset]);
                pOffset += 2;
                size -= 8;
            }
            ...
            {
                for (int j = 0; j < 4; j++) data[4 * i + j] = (byte)(p[i] >> (8 * j));
            }
        }

        public void EncryptArm9(uint cardheader_gamecode, byte[] data)
        {
            uint[] p = new uint[0x800 / 4];
            for (int i = 0; i < p.Length; i++) p[i] = BitConverter.ToUInt32(data, 4 * i);
            ...
            while (size > 0)
            {
                this.Encrypt(card_hash, ref p[pOffset + 1], ref p[pOffset]);
                pOffset += 2;
                size -= 8;
            }
            ...
            p[0] = MAGIC30;
            p[1] = MAGIC34;
            this.Encrypt(card_hash, ref p[1], ref p[0]);
            this.Init1(cardheader_gamecode);
            this.Encrypt(card_hash, ref p[1], ref p[0]);
